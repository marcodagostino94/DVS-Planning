...
function normalizePlanningSlotHeights() {
  for (const room of ROOMS) {
    ...
    roomCells.forEach(cell => {
      const spanningSlot = cell.querySelector(".shift-time-slot.spans-two");
      if (!spanningSlot) return;
      const referenceCell = roomCells.find(candidate =>
        candidate.querySelector('.shift-time-slot[data-slot-index="0"]')
        && candidate.querySelector('.shift-time-slot[data-slot-index="1"]')
      );
      const firstHeight = Number.parseFloat(referenceCell?.querySelector('.shift-time-slot[data-slot-index="0"]')?.style.height) || 88;
      const secondHeight = Number.parseFloat(referenceCell?.querySelector('.shift-time-slot[data-slot-index="1"]')?.style.height) || 88;
      spanningSlot.style.height = `${firstHeight + secondHeight + 5}px`;
    });
  }
}
...
function isStandardSplitShift(shift) {
  const range = `${String(shift.start || "").trim()}-${String(shift.end || "").trim()}`;
  ...
  ROOMS.forEach((room, roomIndex) => {
    ...
    for (const meta of dateMeta) {
      ...
      const spanningHeight = spansTwoSlots
        ...
        : 0;
      const alignedCards = spansTwoSlots
        ? `<div class="shift-time-slot spans-two" data-span-slots="2" data-base-height="${spanningHeight}" style="min-height:${spanningHeight}px">${renderCard(dayShifts[0])}</div>`
        : dayShifts.length ? alignedShifts.map((shift, slotIndex) =>
...
document.querySelectorAll("[data-settings-section]").forEach(button => button.addEventListener("click", () => {
  ...
  const sections = {
    ...
    info: { title:"Informazioni", subtitle:"DVS Planning", html:`<img class="settings-info-logo" src="./assets/logos/digital-video-full.png" alt="Digital Video"><h2>DVS Planning</h2><p>Applicazione collaborativa per la gestione del Planning di Digital Video Service.</p><div class="settings-info-meta"><div><span>Versione</span><strong>v35.2</strong></div><div><span>Ideazione e sviluppo</span><strong>Marco D'Agostino per Digital Video Service</strong></div><div><span>Sincronizzazione</span><strong>Supabase Realtime</strong></div></div><p class="settings-info-copyright"><strong>Copyright © 2026 Marco D'Agostino per Digital Video Service</strong><br>Tutti i diritti riservati.</p>` }
  };
...
// v35.2 — variation uses existing notes and one atomic multi-row upsert.
